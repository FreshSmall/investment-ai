...
小节职责：
- overview：行业定位与当前景气状态（一段话）
- supply_demand：供需与价格的关键边际变化（证据导向）
- catalysts：正在兑现/临近的催化剂
- risks：核心风险（必须与已知风险不同或显著恶化/缓解才更新）
- metrics：跟踪指标清单（仅当需要增删指标时更新）
...
{
  "sections": [
    {"name": "小节枚举名", "content": "更新后的节全文（markdown 纯文本，无标题）", "changed": true, "based_on_event_ids": ["16位hex"]}
  ],
  "changelog_rows": [
